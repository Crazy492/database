...
export default {
  ...
  created() {
    this.$axios.get("/admin/getCollegeCount?page=1&rows=10").then(res => {
      if (res.data.data) {
  ...
  methods: {
    currentChange(val) {
      this.$axios
        .get(`/admin/getCollegeCount?page=${val}&rows=10`)
        .then(res => {
          if (res.data.data) {
            this.tableData = res.data.data.rows;
            this.total = res.data.data.total;
